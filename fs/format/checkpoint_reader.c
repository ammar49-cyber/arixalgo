...
int arix_ckpt_write_close(void* handle) { (void)handle; return 0; }
...
int arix_ckpt_read_close(void* handle) { (void)handle; return 0; }

int arix_ckpt_validate(const char* path) { (void)path; return 0; }
int arix_ckpt_supports_version(uint32_t version) { (void)version; return 1; }
