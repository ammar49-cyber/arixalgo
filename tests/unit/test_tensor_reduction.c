...
static ArixTensor* make_arange_2d(void) {
    size_t shape[] = {2, 3};
...
static void test_sum_all(void) {
    ArixTensor* t = make_arange_2d();
    ArixTensor* s = arix_tensor_sum(t, 0);
...
static void test_mean_basic(void) {
    ArixTensor* t = make_arange_2d();
    ArixTensor* m = arix_tensor_mean(t, 0);
    ...
    arix_tensor_destroy(t); arix_tensor_destroy(m);
}
...
static void test_min_max_basic(void) {
    ...
    ASSERT_NEAR(arix_tensor_min(t), -5.0f, 1e-6f, "min==-5");
    ASSERT_NEAR(arix_tensor_max(t), 7.0f, 1e-6f, "max==7");
...
static void test_argmin_argmax_basic(void) {
    ...
    ArixTensor* t = arix_tensor_create(shape, 1, ARIX_FLOAT32);
    memcpy(t->data, data, 5 * sizeof(float));
    ASSERT(arix_tensor_argmin(t) == 4, "argmin==4 (-5)");
    ASSERT(arix_tensor_argmax(t) == 2, "argmax==2 (7)");
    arix_tensor_destroy(t);
}

static void test_cumsum_basic(void) {
    size_t shape[] = {4};
    ArixTensor* t = arix_tensor_arange(1.0f, 5.0f, 1.0f, ARIX_FLOAT32);
    ArixTensor* c = arix_tensor_cumsum(t, 0);
    ...
    arix_tensor_destroy(t); arix_tensor_destroy(c);
}

static void test_cumprod_basic(void) {
    size_t shape[] = {4};
    ArixTensor* t = arix_tensor_arange(1.0f, 5.0f, 1.0f, ARIX_FLOAT32);
    ArixTensor* c = arix_tensor_cumprod(t, 0);
...
static void test_dot_basic(void) {
    ...
    ArixTensor* b = arix_tensor_arange(1.0f, 4.0f, 1.0f, ARIX_FLOAT32);
    float d = arix_tensor_dot(a, b);
...
static void test_transpose_basic(void) {
    ...
    ArixTensor* t = arix_tensor_create(shape, 2, ARIX_FLOAT32);
    memcpy(t->data, data, 6 * sizeof(float));
    ArixTensor* tr = arix_tensor_transpose(t, 0, 1);
    ...
    ASSERT_NEAR(arix_tensor_get_f32(tr, idx00), 1.0f, 1e-6f, "T[0,0]==1");
    ASSERT_NEAR(arix_tensor_get_f32(tr, idx01), 4.0f, 1e-6f, "T[0,1]==4");
    ASSERT_NEAR(arix_tensor_get_f32(tr, idx20), 3.0f, 1e-6f, "T[2,0]==3");
    ASSERT_NEAR(arix_tensor_get_f32(tr, idx21), 6.0f, 1e-6f, "T[2,1]==6");
    arix_tensor_destroy(t); arix_tensor_destroy(tr);
}
...
static void test_inverse_2x2(void) {
    ...
    ArixTensor* t = arix_tensor_create(shape, 2, ARIX_FLOAT32);
    memcpy(t->data, data, 4 * sizeof(float));
    ArixTensor* inv = arix_tensor_inverse(t);
    ...
    arix_tensor_destroy(t); arix_tensor_destroy(inv);
}
...
static void test_det_2x2(void) {
    ...
    ArixTensor* t = arix_tensor_create(shape, 2, ARIX_FLOAT32);
    memcpy(t->data, data, 4 * sizeof(float));
    ASSERT_NEAR(arix_tensor_det(t), 10.0f, 1e-5f, "det(2x2)==10");
    arix_tensor_destroy(t);
}

static void test_inverse_null(void) {
    ASSERT(arix_tensor_inverse(NULL) == NULL, "inverse null returns NULL");
    ASSERT(arix_tensor_matmul(NULL, NULL) == NULL, "matmul null returns NULL");
    ASSERT(arix_tensor_dot(NULL, NULL) == 0.0f, "dot null returns 0");
    ASSERT(arix_tensor_transpose(NULL, 0, 1) == NULL, "transpose null returns NULL");
    ASSERT(arix_tensor_sum(NULL, 0) == NULL, "sum null returns NULL");
    ASSERT(arix_tensor_mean(NULL, 0) == NULL, "mean null returns NULL");
    ASSERT(arix_tensor_min(NULL) == 0.0f, "min null returns 0");
}
