...
static void test_save_load_basic(void) {
    ...
    int ret = arix_tensor_save(t, "test_tensor.bin");
    ASSERT(ret == 0, "save returns 0");
    ArixTensor* loaded = arix_tensor_load("test_tensor.bin");
    ...
    ASSERT(loaded->dtype == ARIX_FLOAT32, "loaded dtype F32");
    float* ld = (float*)loaded->data;
    for (size_t i = 0; i < 6; i++) ASSERT_NEAR(ld[i], data[i], 1e-6f, "loaded data matches");
    arix_tensor_destroy(t); arix_tensor_destroy(loaded);
...
static void test_save_load_f64(void) {
    ...
    ArixTensor* t = arix_tensor_create(shape, 2, ARIX_FLOAT64);
    memcpy(t->data, data, 4 * sizeof(double));
    ASSERT(arix_tensor_save(t, "test_f64.bin") == 0, "save f64");
    ArixTensor* loaded = arix_tensor_load("test_f64.bin");
    ASSERT(loaded != NULL, "load f64");
    ASSERT(loaded->dtype == ARIX_FLOAT64, "loaded dtype F64");
...
static void test_save_load_null(void) {
    ASSERT(arix_tensor_save(NULL, "test.bin") == -1, "save null returns -1");
    ASSERT(arix_tensor_load(NULL) == NULL, "load null returns NULL");
    ASSERT(arix_tensor_load("nonexistent.bin") == NULL, "load nonexistent returns NULL");
}
...
static void test_cast_f32_to_f64(void) {
    ...
    ArixTensor* c = arix_tensor_cast(t, ARIX_FLOAT64);
    ASSERT(c != NULL, "cast result not null");
    ASSERT(c->dtype == ARIX_FLOAT64, "cast to F64");
...
static void test_cast_f64_to_f32(void) {
    ...
    ArixTensor* t = arix_tensor_create(shape, 1, ARIX_FLOAT64);
    memcpy(t->data, data, 3 * sizeof(double));
    ArixTensor* c = arix_tensor_cast(t, ARIX_FLOAT32);
    ASSERT(c != NULL, "cast result not null");
    ASSERT(c->dtype == ARIX_FLOAT32, "cast to F32");
...
static void test_cast_f32_to_i32(void) {
    ...
    ArixTensor* t = arix_tensor_create(shape, 1, ARIX_FLOAT32);
    memcpy(t->data, data, 3 * sizeof(float));
    ArixTensor* c = arix_tensor_cast(t, ARIX_INT32);
    ASSERT(c != NULL, "cast result not null");
    ASSERT(c->dtype == ARIX_INT32, "cast to INT32");
...
static void test_cast_identity(void) {
    ...
    ASSERT(c->dtype == ARIX_FLOAT32, "identity dtype");
    float* d = (float*)c->data;
...
static void test_to_device_cpu(void) {
    ...
    ArixTensor* d = arix_tensor_to_device(t, ARIX_DEVICE_CPU);
    ASSERT(d != NULL, "to_device result not null");
    ASSERT(d->device == ARIX_DEVICE_CPU, "device == CPU");
    ASSERT(d != t, "to_device returns copy");
    arix_tensor_destroy(t); arix_tensor_destroy(d);
}

static void test_to_device_null(void) {
    ASSERT(arix_tensor_to_device(NULL, ARIX_DEVICE_CPU) == NULL, "to_device null");
}
