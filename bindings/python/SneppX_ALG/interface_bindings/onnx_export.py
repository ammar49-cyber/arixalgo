...
class OnnxInitializer:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "data": self.data.tobytes().hex() if self.data.dtype in [np.float32, np.float64, np.int32, np.int64] else self.data.tobytes(),
        }
...
class OnnxModel:
    ...
    def __init__(
        ...
        self.graph = graph
        self.opset_imports = opset_imports or [{"version": ONNX_OPSET_VERSION, "domain": ""}]
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "metadata_props": [{"key": k, "value": v} for k, v in self.metadata_props.items()],
        }
...
class OnnxExporter:
    ...
    def add_value_info(self, name: str, shape: List[int], dtype: str = "float32") -> str:
        tensor = OnnxTensor(name, dtype, shape)
    ...
    def add_batch_norm(
        ...
        return self.add_node("BatchNormalization", [x, scale, bias, mean, var], attributes=attrs, name=name)[0]
    ...
    def add_layer_norm(
        ...
        return self.add_node("LayerNormalization", [x, weight, bias], attributes=attrs, name=name)[0]
    
    def add_dropout(self, x: str, ratio: float = 0.5, name: Optional[str] = None) -> str:
        attrs = {"ratio": ratio}
    ...
    def add_concat(self, inputs: List[str], axis: int = 1, name: Optional[str] = None) -> str:
    ...
    def add_reshape(self, x: str, shape: List[int], name: Optional[str] = None) -> str:
        ...
        shape_name = self.add_constant(f"{x}_shape", np.array(shape, dtype=np.int64), "int64")
    ...
    def add_squeeze(self, x: str, axes: Optional[List[int]] = None, name: Optional[str] = None) -> str:
        attrs = {}
        if axes:
            attrs["axes"] = axes
    ...
    def add_gather(self, x: str, indices: str, axis: int = 0, name: Optional[str] = None) -> str:
        return self.add_node("Gather", [x, indices], attributes={"axis": axis}, name=name)[0]
    
    def add_reduce_sum(self, x: str, axes: List[int], keepdims: int = 0, name: Optional[str] = None) -> str:
        return self.add_node("ReduceSum", [x], attributes={"axes": axes, "keepdims": keepdims}, name=name)[0]
    
    def add_reduce_mean(self, x: str, axes: List[int], keepdims: int = 0, name: Optional[str] = None) -> str:
        return self.add_node("ReduceMean", [x], attributes={"axes": axes, "keepdims": keepdims}, name=name)[0]
    
    def add_split(self, x: str, split: List[int], axis: int = 0, name: Optional[str] = None) -> List[str]:
    ...
    def add_leaky_relu(self, x: str, alpha: float = 0.01, name: Optional[str] = None) -> str:
        return self.add_node("LeakyRelu", [x], attributes={"alpha": alpha}, name=name)[0]
...
ONNX_OP_REGISTRY = {
    "Conv": {"inputs": 2, "outputs": 1, "attrs": ["strides", "pads", "dilations", "group"]},
    ...
    "MaxPool": {"inputs": 1, "outputs": 1, "attrs": ["kernel_shape", "strides", "pads"]},
    "AveragePool": {"inputs": 1, "outputs": 1, "attrs": ["kernel_shape", "strides", "pads"]},
...
def onnx_version_compatible(onnx_model: OnnxModel, target_opset: int = ONNX_OPSET_VERSION) -> bool:
