...
from .advanced_ops import (
    linear, layernorm, gelu, relu, dropout, softmax, multi_head_attention,
    conv2d, adaptive_avg_pool2d, flatten, cat
)
...
class VisionTransformer(Module):
    ...
    def __init__(
        ...
        self.pos_embed = Tensor.from_numpy(
            np.random.normal(0, 0.02, (1, num_patches + 1, embed_dim)).astype(np.float32)
        )
...
class ViTMoEBlock(Module):
    ...
    def __init__(self, dim: int, num_heads: int, mlp_ratio: float = 4.0,
                 num_experts: int = 8, top_k: int = 2, dropout: float = 0.0):
        super().__init__()
        self.norm1 = layernorm(dim)
...
class WindowAttention(Module):
    ...
    def forward(self, x: Tensor, H: int, W: int) -> Tensor:
        ...
        qkv = self.qkv(x).reshape(B, L, 3, self.num_heads, self.dim // self.num_heads).permute(2, 0, 3, 1, 4)
...
class MAE(Module):
    ...
    def __init__(
        ...
        self.decoder_pos_embed = Tensor.from_numpy(
            np.random.normal(0, 0.02, (1, num_patches + 1, decoder_embed_dim)).astype(np.float32)
        )
        
        self.decoder_blocks = Sequential([
            ViTBlock(decoder_embed_dim, 8, 4.0, 0.0, 0.0) for _ in range(decoder_depth)
        ])
...
def mae_large(**kwargs) -> MAE:
    return MAE(embed_dim=1024, encoder_depth=24, decoder_depth=8, num_heads=16, decoder_embed_dim=512, **kwargs)


def mae_huge(**kwargs) -> MAE:
    return MAE(embed_dim=1280, encoder_depth=32, decoder_depth=16, num_heads=16, decoder_embed_dim=512, **kwargs)
...
def create_vision_model(
    ...
    **kwargs
    ...
    models = {
        ...
        # DeiT
        'deit_tiny': lambda: DeiTDistilled(img_size=224, patch_size=16, embed_dim=192, depth=12, num_heads=3, num_classes=num_classes),
        'deit_small': lambda: DeiTDistilled(img_size=224, patch_size=16, embed_dim=384, depth=12, num_heads=6, num_classes=num_classes),
        'deit_base': lambda: DeiTDistilled(img_size=224, patch_size=16, embed_dim=768, depth=12, num_heads=12, num_classes=num_classes),
        
        # Swin
        'swin_tiny': lambda: SwinTransformer(embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), num_classes=num_classes),
        'swin_small': lambda: SwinTransformer(embed_dim=96, depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24), num_classes=num_classes),
        'swin_base': lambda: SwinTransformer(embed_dim=128, depths=(2, 2, 18, 2), num_heads=(4, 8, 16, 32), num_classes=num_classes),
        'swin_large': lambda: SwinTransformer(embed_dim=192, depths=(2, 2, 18, 2), num_heads=(6, 12, 24, 48), num_classes=num_classes),
...
__all__ = [
    ...
    'ViTModel', 'ViTForMaskedLM', 'ViTForImageClassification',
    'ViTConfig',
    'vit_tiny_patch16_224', 'vit_small_patch16_224', 'vit_base_patch16_224',
    'vit_large_patch16_224', 'vit_huge_patch14_224',
    'DeiTDistilled', 'deit_tiny_patch16_224', 'deit_small_patch16_224',
    'deit_base_patch16_224',
    'SwinTransformer', 'SwinBlock', 'SwinPatchEmbed', 'SwinPatchMerging',
