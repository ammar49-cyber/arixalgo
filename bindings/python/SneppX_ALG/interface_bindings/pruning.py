...
def global_magnitude_prune(
    ...
    for w, shape in zip(weights, shapes):
        ...
        results.append((
            Tensor.from_numpy(pruned.astype(np.float32)),
            Tensor.from_numpy(mask)
        ))
...
class CubicSparsityScheduler(SparsityScheduler):
    ...
    def get_sparsity(self) -> float:
        ...
        # Cubic easing
        coeff = 3 * t ** 2 - 2 * t ** 3
        return self.initial_sparsity + (self.final_sparsity - self.initial_sparsity) * coeff
...
def print_pruning_summary(model_params: List[Tensor], names: Optional[List[str]] = None):
    """Print pruning summary for model parameters."""
    print(f"{'Layer':<30} {'Shape':<20} {'Total':>10} {'Non-zero':>10} {'Sparsity':>10}")
    print("-" * 80)
    ...
    print("-" * 80)
    print(f"{'Total':<30} {'':<20} {total_params:>10} {total_nonzero:>10} {total_sparsity*100:>9.2f}%")
...
def distillation_loss(
    ...
    kl_loss = np.sum(soft_teacher * (np.log(soft_teacher + 1e-12) - soft_student), axis=-1)
