...
try:
    ...
    from ..advanced_ops import (
        ...
        linear, layernorm, rmsnorm, gelu, silu, relu,
        softmax, batch_norm, layer_norm, group_norm,
        matmul, add, mul, matmul_batched, matmul_strided
    )
except ImportError:
...
class BenchmarkSuite:
    ...
    def benchmark_matmul(self, shapes: List[Tuple[int, int, int]] = None) -> List[BenchmarkResult]:
        """Benchmark matrix multiplication."""
        if shapes is None:
            shapes = [
                (64, 64, 64), (128, 128, 128), (256, 256, 256),
                (512, 512, 512), (1024, 1024, 1024),
                (64, 2048, 512), (512, 64, 2048),  # attention shapes
            ]
    ...
    def benchmark_transformer_block(self, configs: List[Dict] = None) -> List[BenchmarkResult]:
        ...
        for cfg in configs:
            ...
            class MockModule:
                def __init__(self):
                    self.w_qkv = Tensor.from_numpy(np.random.randn(3*D, D).astype(np.float32))
                    self.w_o = Tensor.from_numpy(np.random.randn(D, D).astype(np.float32))
    ...
    def benchmark_norms(self, shapes: List[Tuple[int, ...]] = None) -> List[BenchmarkResult]:
        """Benchmark normalization layers."""
        if shapes is None:
            shapes = [
                (32, 512), (64, 768), (32, 1024), (16, 2048),
                (4, 32, 56, 56), (8, 64, 28, 28),
            ]
        ...
        for shape in shapes:
            ...
            for name, fn in [("layernorm", layernorm), ("rmsnorm", rmsnorm),
                ...
                result = self.benchmark(
                    ...
                    lambda: fn(x, Tensor.from_numpy(np.ones(C, dtype=np.float32)),
    ...
    def run_full_suite(self) -> Dict[str, List[BenchmarkResult]]:
        ...
        print(f"Python: {self._device_info['python']}, NumPy: {self._device_info['numpy']}")
        print(f"{'='*60}\n")
    ...
    def print_summary(self):
        ...
        for cat, results in sorted(categories.items()):
            ...
            for r in sorted(results, key=lambda x: x.mean_time_ms):
                print(f"  {r.name:40s} {r.mean_time_ms:>8.2f} ms  "
...
def run_benchmark_cli():
    ...
    parser.add_argument("--suite", choices=["all", "matmul", "conv", "attention", 
                                            "transformer", "activations", "norms"],
