...
class BenchmarkSuite:
    ...
    def benchmark_attention(self, configs: Optional[List[Dict]] = None) -> List[BenchmarkResult]:
        ...
        for cfg in configs:
            ...
            result = self.benchmark(
                f"attention_B{B}_L{L}_D{D}_H{H}", "attention",
                lambda: multi_head_attention(q, k, v, num_heads=H, dropout_p=0.1, training=True),
                memory_mb=mem_mb
            )
            results.append(result)
        return results
    
    def benchmark_rnn(self, configs: Optional[List[Dict]] = None) -> List[BenchmarkResult]:
        ...
        for cfg in configs:
            ...
            result = self.benchmark(
                f"rnn_{cfg['type']}_B{B}_S{S}_I{I}_H{H}", "rnn",
                fn, memory_mb=mem_mb
            )
            results.append(result)
        return results
    ...
    def benchmark_transformer_block(self, configs: Optional[List[Dict]] = None) -> List[BenchmarkResult]:
        ...
        if configs is None:
            configs = [
                ...
                {"batch": 4, "seq": 256, "dim": 1024, "heads": 16, "ff_mult": 4, "dropout": 0.1},
            ]
        ...
        for cfg in configs:
            ...
            mask = Tensor.from_numpy(np.triu(np.ones((L, L)) * -1e9, k=1).astype(np.float32))
            
            mem_mb = (B * L * D * 4 + B * L * L * 4 + B * L * cfg["ff_mult"] * D * 4) / (1024*1024)
            
            result = self.benchmark(
                f"transformer_B{B}_L{L}_D{D}_H{H}", "transformer",
                lambda: transformer_block(x, mask, num_heads=H, ff_mult=cfg["ff_mult"], dropout=cfg["dropout"]),
                memory_mb=mem_mb
            )
            results.append(result)
        return results
    
    def benchmark_activation(self, activations: Optional[List[str]] = None) -> List[BenchmarkResult]:
        """Benchmark activation functions."""
        if activations is None:
            activations = ["relu", "gelu", "silu", "tanh", "sigmoid", "relu6", "hardswish", "leaky_relu", "elu", "selu", "softplus", "softsign", "mish"]
        
        from .advanced_ops import relu, gelu, silu, tanh, sigmoid, relu6, hardswish, leaky_relu, elu, selu, softplus, softsign, mish
        act_fns = {
    ...
    def benchmark_reduction(self, shapes: Optional[List[Tuple[int, ...]]] = None) -> List[BenchmarkResult]:
        ...
        for shape in shapes:
            ...
            for op_name, fn in [
                ...
                result = self.benchmark(
                    f"{op_name}_{shape}", "reduction",
                    fn, memory_mb=mem_mb
                )
                results.append(result)
        return results
    
    def benchmark_pointwise(self, shapes: Optional[List[Tuple[int, ...]]] = None) -> List[BenchmarkResult]:
        ...
        for shape in shapes:
            ...
            for op_name, fn in [
                ...
                result = self.benchmark(
                    f"{op_name}_{shape}", "pointwise",
                    fn, memory_mb=mem_mb
                )
                results.append(result)
        return results
    
    def benchmark_norm(self, shapes: Optional[List[Tuple[int, ...]]] = None) -> List[BenchmarkResult]:
        """Benchmark normalization operations."""
        if shapes is None:
            shapes = [(1000, 1000), (100, 10000), (10, 100000)]
        
        from .advanced_ops import layernorm, rmsnorm, batch_norm, group_norm, instance_norm, layer_norm
        results = []
        for shape in shapes:
            x = Tensor.from_numpy(np.random.randn(*shape).astype(np.float32))
            ...
            for op_name, fn in [
                ...
                result = self.benchmark(
                    f"{op_name}_{shape}", "normalization",
                    fn, memory_mb=mem_mb
                )
                results.append(result)
        return results
    
    def benchmark_pooling(self, shapes: Optional[List[Tuple[int, ...]]] = None) -> List[BenchmarkResult]:
        """Benchmark pooling operations."""
        if shapes is None:
            shapes = [(1, 64, 224, 224), (4, 128, 56, 56), (8, 256, 28, 28)]
        
        from .advanced_ops import max_pool2d, avg_pool2d, adaptive_avg_pool2d, adaptive_max_pool2d
        results = []
        for shape in shapes:
            x = Tensor.from_numpy(np.random.randn(*shape).astype(np.float32))
            ...
            for op_name, fn in [
                ...
                result = self.benchmark(
                    f"{op_name}_{shape}", "pooling",
                    fn, memory_mb=mem_mb
                )
                results.append(result)
        return results
    
    def save_results(self, output_dir: Optional[str] = None) -> Tuple[Optional[str], Optional[str]]:
        ...
        return json_path if self.config.save_json else None, csv_path if self.config.save_csv else None
