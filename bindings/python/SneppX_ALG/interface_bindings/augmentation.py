    ...
    if interpolation == "nearest":
        ...
        if img.ndim == 3:  # C, H, W
            ...
            for i in range(c):
                ...
                out[i] = (v00 * (1 - h_weight[:, None]) * (1 - w_weight[None, :]) +
            ...
            return (v00 * (1 - h_weight[:, None]) * (1 - w_weight[None, :]) +
                    v01 * (1 - h_weight[:, None]) * w_weight[None, :] +
                    v10 * h_weight[:, None] * (1 - w_weight[None, :]) +
                    v11 * h_weight[:, None] * w_weight[None, :])
...
def gaussian_blur(img: Tensor, kernel_size: int = 3, sigma: Tuple[float, float] = (0.1, 2.0)) -> Tensor:
    """Gaussian blur."""
    x = np.asarray(img.data, dtype=np.float64)
...
def random_token_mask(
    ...
    return Tensor.from_numpy(input_ids_np.astype(np.int64)), Tensor.from_numpy(labels.astype(np.int64))
...
def random_word_dropout(
    ...
    input_ids_np = np.asarray(input_ids.data, dtype=np.int64).copy()
    mask = (np.random.random(input_ids_np.shape) < dropout_prob) & (input_ids_np != pad_token_id)
...
class RandomOrder:
    ...
    def __call__(self, *args, **kwargs):
        ...
        for i in order:
            args = self.transforms[i](*args, **kwargs) if not isinstance(args, tuple) else self.transforms[i](*args)
        return args
...
def center_crop(img: Tensor, size: Tuple[int, int]) -> Tensor:
    ...
    top = (h - new_h) // 2
    left = (w - new_w) // 2
    return Tensor.from_numpy(x[..., top:top+new_h, left:left+new_w].astype(np.float32))
...
def random_crop(
    ...
    if h < new_h or w < new_w:
        if pad_if_needed:
            pad_h = max(0, new_h - h)
            pad_w = max(0, new_w - w)
            x = np.pad(x, ((0,0), (pad_h//2, pad_h - pad_h//2), (pad_w//2, pad_w - pad_w//2)), mode='constant')
            h, w = x.shape[-2:]
        else:
            return Tensor.from_numpy(x.astype(np.float32))
...
def five_crop(img: Tensor, size: Tuple[int, int]) -> List[Tensor]:
    ...
    positions = [
        (0, 0), (0, w - crop_w), (h - crop_h, 0), (h - crop_h, w - crop_w),
        ((h - crop_h) // 2, (w - crop_w) // 2),
    ]
    
    for top, left in positions:
        crops.append(Tensor.from_numpy(x[..., top:top+crop_h, left:left+crop_w].astype(np.float32)))
