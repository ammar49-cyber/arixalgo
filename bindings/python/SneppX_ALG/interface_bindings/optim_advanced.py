        ...
        for i, p in enumerate(self.params):
            ...
            if arr.ndim == 2:
                ...
                if self.momentum > 0 and s["momentum"] is not None:
                    s["momentum"] = self.momentum * s["momentum"] + (1 - self.momentum) * precond
                    update = s["momentum"]
                else:
                    update = precond
            else:
...
class CaProp:
    ...
    def step(self):
        ...
        for i, p in enumerate(self.params):
            ...
            alignment = np.sum(g * m_hat) / (np.linalg.norm(g) * np.linalg.norm(m_hat) + self.eps)
        ...
        for i, p in enumerate(self.params):
            ...
            if "L" in s and self.step_count % self.precond_freq == 0:
                ...
                if g.ndim == 2:
                    ...
                    L_inv_sqrt = np.linalg.inv(np.sqrt(L + self.eps * np.eye(L.shape[0])))
                    R_inv_sqrt = np.linalg.inv(np.sqrt(R + self.eps * np.eye(R.shape[0])))
    ...
    def step(self):
        ...
        for i, p in enumerate(self.params):
            ...
            if "partition_start" in s:
                ...
                exp_avg_sq = self.beta2 * exp_avg_sq + (1 - self.beta2) * (g_flat * g_flat)
...
def get_optimizer(name: str, params: List[Tensor], **kwargs) -> Any:
    ...
    if name not in registry:
        raise ValueError(f"Unknown optimizer: {name}. Available: {list(registry.keys())}")
