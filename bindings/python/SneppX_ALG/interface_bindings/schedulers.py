...
class LRScheduler:
    ...
    def __init__(self, optimizer, last_epoch: int = -1):
        ...
        self.base_lrs = list(getattr(optimizer, "param_groups", [{"lr": getattr(optimizer, "lr", 1e-3)}]))
        if not self.base_lrs:
            self.base_lrs = [1e-3]
        # If base_lrs is a list of dicts (param_groups), extract the lr values
        if self.base_lrs and isinstance(self.base_lrs[0], dict):
            self.base_lrs = [g.get("lr", 1e-3) for g in self.base_lrs]
...
class LinearWarmupCosineDecay(LRScheduler):
    ...
    def get_lr(self) -> float:
        ...
        return self.min_lr + 0.5 * (base - self.min_lr) * (1 + math.cos(math.pi * progress))


class PolynomialLR(LRScheduler):
    """Polynomial decay: ``lr = base * (1 - t/T)^power``."""

    def __init__(self, optimizer, total_steps: int, power: float = 2.0,
                 lr_end: float = 0.0, last_epoch: int = -1):
        self.total_steps = max(1, total_steps)
        self.power = power
        self.lr_end = lr_end
        super().__init__(optimizer, last_epoch)
    ...
    def get_lr(self) -> float:
        ...
        return (self.base_lrs[0] - self.lr_end) * ((1 - t / self.total_steps) ** self.power) + self.lr_end
...
class ReduceLROnPlateau(LRScheduler):
    ...
    def __init__(self, optimizer, mode: str = "min", factor: float = 0.1,
                 ...
                 min_lr: float = 0.0, eps: float = 1e-8, last_epoch: int = -1):
        self.mode = mode
        self.factor = factor
...
class CosineAnnealingWithWarmupRestarts(LRScheduler):
    ...
    def __init__(self, optimizer, warmup_steps: int, T_0: int, T_mult: int = 1,
                 eta_min: float = 0.0, last_epoch: int = -1):
        self.warmup = ConstantLRWithWarmup(optimizer, warmup_steps, last_epoch - 1)
        self.cosine = CosineAnnealingWarmRestarts(optimizer, T_0, T_mult, eta_min, last_epoch)
        super().__init__(optimizer, last_epoch)
...
class SequentialLR(LRScheduler):
    ...
    def __init__(self, optimizer, schedulers: List[LRScheduler],
                 milestones: List[int], last_epoch: int = -1):
...
class TriStageLR(LRScheduler):
    ...
    def __init__(self, optimizer, total_steps: int, warmup_fraction: float = 0.1,
                 decay_fraction: float = 0.1, last_epoch: int = -1):
        self.total_steps = max(1, total_steps)
...
class CosineAnnealingWithWarmup(LRScheduler):
    ...
    def get_lr(self) -> float:
        ...
        progress = (step - self.warmup_steps) / max(1, self.total_steps - self.warmup_steps)
        progress = min(1.0, max(0.0, progress))
        return self.eta_min + 0.5 * (base - self.eta_min) * (1 + math.cos(math.pi * progress))
