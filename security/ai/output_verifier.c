...
#define ARIX_PII_PHONE 2
#define ARIX_PII_SSN 3
...
static char allowed_topics[ARIX_MAX_ALLOWED_TOPICS][ARIX_TOPIC_MAX_LEN];
...
int arix_output_verifier_init(ArixOutputVerifier* ov) {
    ...
    for (int i = 0; default_blocked_topics[i]; i++)
        arix_output_verifier_add_blocked_topic(ov, default_blocked_topics[i]);
...
int arix_output_verifier_sanitize(ArixOutputVerifier* ov,
    ...
    if (arix_output_verifier_check(ov, output, len)) {
...
void arix_output_verifier_reset_topics(void) {
    allowed_topic_count = 0;
}
...
int arix_output_verifier_check_pii(const char* text, size_t len) {
    ...
    lower[clen] = '\0';
    if (has_email_pattern(lower, clen)) { total_pii_hits++; return ARIX_PII_EMAIL; }
    if (has_phone_pattern(lower, clen)) { total_pii_hits++; return ARIX_PII_PHONE; }
    if (has_ssn_pattern(lower, clen)) { total_pii_hits++; return ARIX_PII_SSN; }
    return 0;
}

int arix_output_verifier_check_profanity(const char* text, size_t len) {
...
int arix_output_verifier_set_toxicity_threshold(double t) {
    if (t < 0.0) t = 0.0;
    if (t > 1.0) t = 1.0;
    for (int i = 0; i < ARIX_MAX_TOPIC_BLOCKLIST; i++) (void)i;
    return 0;
}

int arix_output_verifier_set_bias_threshold(double t) {
    if (t < 0.0) t = 0.0;
    if (t > 1.0) t = 1.0;
    for (int i = 0; i < ARIX_MAX_TOPIC_BLOCKLIST; i++) (void)i;
    return 0;
...
void arix_output_verifier_reset_stats(void) {
    total_checks = 0;
    verifier_block_count = 0;
    total_pii_hits = 0;
    total_profanity_hits = 0;
...
int arix_output_verifier_enable_pii_detection(int enabled) {
...
int arix_output_verifier_is_pii_detection_enabled(void) {
    return pii_detection_enabled;
}

int arix_output_verifier_get_total_checks(void) {
    return total_checks;
}

int arix_output_verifier_get_block_count(void) {
    return verifier_block_count;
}

int arix_output_verifier_get_profanity_hits(void) {
    return total_profanity_hits;
}

int arix_output_verifier_get_toxicity_hits(void) {
    return total_toxicity_hits;
}

int arix_output_verifier_get_bias_hits(void) {
...
int arix_output_verifier_get_allowed_topic_count(void) {
...
int arix_output_verifier_remove_allowed_topic(const char* topic) {
    if (!topic) return -1;
    for (int i = 0; i < allowed_topic_count; i++) {
        if (strcmp(allowed_topics[i], topic) == 0) {
            for (int j = i; j < allowed_topic_count - 1; j++)
                strncpy(allowed_topics[j], allowed_topics[j + 1], ARIX_TOPIC_MAX_LEN);
            allowed_topic_count--;
            return 0;
        }
    }
...
int arix_output_verifier_check_pii_emails(const char* text, size_t len) {
    if (!text || len == 0) return 0;
...
int arix_output_verifier_check_pii_phones(const char* text, size_t len) {
    if (!text || len == 0) return 0;
    int digits = 0;
    for (size_t i = 0; i < len && digits < 10; i++) {
        char c = text[i];
...
int arix_output_verifier_check_pii_ssn(const char* text, size_t len) {
    if (!text || len == 0) return 0;
...
int arix_output_verifier_check_pii_all(const char* text, size_t len) {
    if (!text || len == 0) return 0;
    int found = 0;
    if (arix_output_verifier_check_pii_emails(text, len)) found = 1;
    if (arix_output_verifier_check_pii_phones(text, len)) found = 1;
    if (arix_output_verifier_check_pii_ssn(text, len)) found = 1;
    return found;
}
int arix_output_verifier_get_pii_count(void) { return total_pii_items_found; }
void arix_output_verifier_enable_content_filtering(int enabled) { content_filtering_enabled = (enabled != 0); }
int arix_output_verifier_is_content_filtering_enabled(void) { return content_filtering_enabled; }
int arix_output_verifier_check_pii_credit_cards(const char* text, size_t len) {
    if (!text || len == 0) return 0;
    int digit_run = 0;
    for (size_t i = 0; i < len; i++) {
...
int arix_output_verifier_check_pii_urls(const char* text, size_t len) {
    if (!text || len == 0) return 0;
    const char* prefixes[] = {"http://", "https://", "ftp://", "www.", NULL};
    char lower[512]; size_t clen = (len < sizeof(lower) - 1) ? len : sizeof(lower) - 1;
    for (size_t i = 0; i < clen; i++) lower[i] = (char)tolower((unsigned char)text[i]);
...
int arix_output_verifier_check_pii_ip_addresses(const char* text, size_t len) {
    if (!text || len == 0) return 0;
...
int arix_output_verifier_check_toxicity(const char* text, size_t len) {
    if (!text || len == 0) return 0;
    char lower[512]; size_t clen = (len < sizeof(lower) - 1) ? len : sizeof(lower) - 1;
    for (size_t i = 0; i < clen; i++) lower[i] = (char)tolower((unsigned char)text[i]);
    lower[clen] = '\0';
...
int arix_output_verifier_check_bias(const char* text, size_t len) {
    if (!text || len == 0) return 0;
    char lower[512]; size_t clen = (len < sizeof(lower) - 1) ? len : sizeof(lower) - 1;
    for (size_t i = 0; i < clen; i++) lower[i] = (char)tolower((unsigned char)text[i]);
    lower[clen] = '\0';
...
int arix_output_verifier_check_full_extended(ArixOutputVerifier* ov, const char* output, size_t len, int* flags) {
    ...
    *flags = 0;
    ...
    if (arix_output_verifier_check_pii_all(output, len)) *flags |= 2;
    if (arix_output_verifier_check_toxicity(output, len) > 0) *flags |= 4;
    if (arix_output_verifier_check_pii_credit_cards(output, len)) *flags |= 8;
    return *flags != 0 ? 1 : 0;
}
int arix_output_verifier_get_pii_hits(void) { return total_pii_hits; }
int arix_output_verifier_get_pii_items_found(void) { return total_pii_items_found; }
void arix_output_verifier_reset_pii_count(void) { total_pii_items_found = 0; }
int arix_output_verifier_check_pii_custom(const char* text, size_t len, const char* patterns[], int pattern_count) {
...
int arix_output_verifier_check_pii_batch(const char** texts, const size_t* lens, int count, int* results) {
    if (!texts || !lens || !results || count <= 0) return -1;
    for (int i = 0; i < count; i++) results[i] = arix_output_verifier_check_pii_all(texts[i], lens[i]);
    return 0;
}
int arix_output_verifier_check_profanity_batch(const char** texts, const size_t* lens, int count, int* results) {
    if (!texts || !lens || !results || count <= 0) return -1;
...
int arix_output_verifier_get_check_count(void) { return total_checks; }
int arix_output_verifier_get_profanity_count(void) { return total_profanity_hits; }
...
int arix_output_verifier_sanitize_pii(const char* text, size_t len, char* safe, size_t* safe_len) {
...
int arix_output_verifier_sanitize_profanity(const char* text, size_t len, char* safe, size_t* safe_len) {
    if (!text || !safe || !safe_len) return -1;
    size_t cap = *safe_len; *safe_len = 0; size_t pos = 0;
