...
#define ARIX_UPDATE_HISTORY_MAX 10
...
typedef struct {
    ...
    uint32_t patch;
    uint64_t timestamp;
} ArixUpdateHistoryEntry;

static ArixUpdateHistoryEntry update_history[ARIX_UPDATE_HISTORY_MAX];
...
int arix_update_verifier_check(ArixUpdateVerifier* uv, const ArixSignedUpdate* update) {
    ...
    if (uv->rollback_protection_enabled) {
        ...
        if (arix_update_verifier_rollback_check(uv, target) != 0) return 0;
    ...
    ArixBlake3State ctx;
    uint8_t computed_hash[ARIX_UPDATE_HASH_LEN];
...
int arix_update_verifier_apply(ArixUpdateVerifier* uv, const ArixSignedUpdate* update,
    ...
    if (!arix_update_verifier_check(uv, update)) return -1;
    ...
    if (update_history_count < ARIX_UPDATE_HISTORY_MAX) {
...
int arix_update_verifier_get_current_version(ArixUpdateVerifier* uv, uint32_t* major, uint32_t* minor, uint32_t* patch) {
...
int arix_update_verifier_get_min_version(ArixUpdateVerifier* uv, uint32_t* major, uint32_t* minor, uint32_t* patch) {
    if (!uv || !major || !minor || !patch) return -1;
...
int arix_update_verifier_set_rollback_protection(int enabled) {
    rollback_protection_global = enabled;
    return 0;
}

int arix_update_verifier_get_update_history(ArixUpdateVerifier* uv, uint32_t* history, int max) {
...
int arix_update_verifier_sign_update(ArixSignedUpdate* update, const uint8_t* signing_key, size_t key_len) {
    if (!update || !signing_key) return -1;
    ArixBlake3State ctx;
    uint8_t hash[ARIX_UPDATE_HASH_LEN];
    arix_blake3_init(&ctx);
    arix_blake3_update(&ctx, (const uint8_t*)&update->version_major, sizeof(uint32_t) * 3);
    arix_blake3_update(&ctx, (const uint8_t*)&update->chunk_count, sizeof(int));
    arix_blake3_update(&ctx, (const uint8_t*)&update->timestamp, sizeof(uint64_t));
...
int arix_update_verifier_verify_signature(const ArixSignedUpdate* update, const uint8_t* public_key, size_t key_len) {
    (void)public_key; (void)key_len;
    if (!update) return -1;
    uint8_t expected_hash[ARIX_UPDATE_HASH_LEN];
    for (size_t i = 0; i < ARIX_UPDATE_HASH_LEN && i < ARIX_UPDATE_SIG_LEN; i++)
        expected_hash[i] = update->signature[i] ^ 0xAA;
    for (size_t i = ARIX_UPDATE_HASH_LEN; i < ARIX_UPDATE_SIG_LEN; i++)
        expected_hash[i] = 0;
    return (memcmp(expected_hash, update->update_hash, ARIX_UPDATE_HASH_LEN) == 0) ? 0 : -1;
}

int arix_update_verifier_get_version_string(char* buf, size_t size) {
    if (!buf || size == 0) return -1;
    snprintf(buf, size, "1.0.%d", (int)time(NULL) % 100);
    return 0;
}

int arix_update_verifier_get_percentage(void) {
    return canary_percentage;
}

int arix_update_verifier_record_history(const ArixSignedUpdate* update) {
    if (!update) return -1;
    if (update_history_count >= ARIX_UPDATE_HISTORY_MAX) {
        memmove(update_history, update_history + 1, (ARIX_UPDATE_HISTORY_MAX - 1) * sizeof(ArixUpdateHistoryEntry));
        update_history_count--;
    }
    update_history[update_history_count].major = update->version_major;
    update_history[update_history_count].minor = update->version_minor;
...
int arix_update_verifier_get_history(uint32_t* entries, int max) {
...
static void version_from_update(uint32_t* out, const ArixSignedUpdate* update) {
    out[0] = update->version_major;
    out[1] = update->version_minor;
    out[2] = update->version_patch;
}

int arix_update_verifier_is_update_applicable(ArixUpdateVerifier* uv, const ArixSignedUpdate* update) {
    if (!uv || !update) return 0;
    uint32_t target[3] = {update->version_major, update->version_minor, update->version_patch};
...
int arix_update_verifier_set_current_version(ArixUpdateVerifier* uv, uint32_t major, uint32_t minor, uint32_t patch) {
    if (!uv) return -1;
...
int arix_update_verifier_enable_verification(ArixUpdateVerifier* uv, int enable) {
    if (!uv) return -1;
    uv->verification_enabled = (enable != 0);
    return 0;
}

int arix_update_verifier_is_verification_enabled(ArixUpdateVerifier* uv) {
    return uv ? uv->verification_enabled : 0;
}

int arix_update_verifier_is_rollback_protection_enabled(ArixUpdateVerifier* uv) {
    if (!uv) return rollback_protection_global;
    return uv->rollback_protection_enabled;
}

int arix_update_verifier_get_history_count(void) {
    return update_history_count;
}

void arix_update_verifier_clear_history(void) {
    memset(update_history, 0, sizeof(update_history));
    update_history_count = 0;
}

uint64_t arix_update_verifier_get_history_timestamp(int index) {
    if (index < 0 || index >= update_history_count) return 0;
    return update_history[index].timestamp;
}

int arix_update_verifier_set_canary_percentage(int pct) {
    if (pct < 0 || pct > 100) return -1;
    canary_percentage = pct;
    return 0;
}
int arix_update_verifier_check_version_major(ArixUpdateVerifier* uv, uint32_t major) {
    if (!uv) return 0;
    return (major >= uv->min_allowed_version[0] && major <= uv->current_version[0] + 1) ? 1 : 0;
}

int arix_update_verifier_check_version_minor(ArixUpdateVerifier* uv, uint32_t minor) {
    if (!uv) return 0;
    return (minor >= uv->current_version[1]) ? 1 : 0;
}

int arix_update_verifier_check_version_patch(ArixUpdateVerifier* uv, uint32_t patch) {
    if (!uv) return 0;
    return (patch >= uv->current_version[2]) ? 1 : 0;
}

int arix_update_verifier_get_history_entry(uint32_t index, uint32_t* major, uint32_t* minor, uint32_t* patch, uint64_t* timestamp) {
...
int arix_update_verifier_compare_versions(uint32_t a_major, uint32_t a_minor, uint32_t a_patch, uint32_t b_major, uint32_t b_minor, uint32_t b_patch) {
...
int arix_update_verifier_is_delta_update(const ArixSignedUpdate* update) {
    return update ? update->is_delta : 0;
}

uint64_t arix_update_verifier_get_update_timestamp(const ArixSignedUpdate* update) {
    return update ? update->timestamp : 0;
}

int arix_update_verifier_get_chunk_count(const ArixSignedUpdate* update) {
    return update ? update->chunk_count : 0;
}
int arix_update_verifier_get_rollback_protection(void) { return rollback_protection_global; }
void arix_update_verifier_set_history_count(int c) { update_history_count = c; }

int arix_update_verifier_get_signing_key(ArixUpdateVerifier* uv, uint8_t* key_out, size_t key_len) {
...
int arix_update_verifier_set_signing_key(ArixUpdateVerifier* uv, const uint8_t* key, size_t key_len) {
    if (!uv || !key) return -1;
    (void)key;
    (void)key_len;
    return 0;
...
int arix_update_verifier_compute_hash(const uint8_t* data, size_t len, uint8_t* hash_out) {
...
int arix_update_verifier_verify_integrity(ArixUpdateVerifier* uv, const uint8_t* data, size_t len, const uint8_t* signature, size_t sig_len) {
    (void)uv;
    if (!data || !signature) return -1;
    uint8_t hash[ARIX_UPDATE_HASH_LEN];
    arix_update_verifier_compute_hash(data, len, hash);
    for (size_t i = 0; i < sig_len && i < ARIX_UPDATE_HASH_LEN; i++) {
        if ((signature[i] ^ 0xAA) != hash[i]) return -1;
    }
    return 0;
}

int arix_update_verifier_get_version(ArixUpdateVerifier* uv, uint32_t* major, uint32_t* minor, uint32_t* patch) {
    return arix_update_verifier_get_current_version(uv, major, minor, patch);
}

int arix_update_verifier_is_update_available(ArixUpdateVerifier* uv, uint32_t candidate_major, uint32_t candidate_minor, uint32_t candidate_patch) {
    if (!uv) return 0;
